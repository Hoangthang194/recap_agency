...
// Stub types and values for yjs
type YArray<T> = any;
type YMap<T> = any;
type Transaction = any;
type YArrayEvent = any;
type YEvent<T = any> = any;
// Stub constructors that can be used as values
const YArray = class {
  constructor() {}
  push(...items: any[]) {}
  delete(index: number, length: number) {}
  get(index: number) { return null; }
  length = 0;
} as any;
const YMap = class {
  constructor() {}
...
const YArrayEvent = class {} as any;
...
export class CommentStore {
  ...
  _getCollabComments(): null | YArray<any> {
    ...
    if (provider !== null) {
      const doc = (provider as any).doc;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return doc?.get?.('comments', YArray) as YArray<any> || null;
    }
    return null;
  }
  ...
  _createCollabSharedMap(commentOrThread: Comment | Thread): YMap<any> {
    ...
    if (type === 'comment') {
      ...
      commentOrThread.comments.forEach((comment, i) => {
        ...
        (commentsArray as any).insert?.(i, [sharedChildComment]);
      });
