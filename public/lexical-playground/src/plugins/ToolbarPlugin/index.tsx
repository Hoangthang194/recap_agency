...
import {
  $isCodeNode,
  getCodeLanguages,
  normalizeCodeLang as normalizeCodeLanguagePrism,
} from '@lexical/code';
// Stub for @lexical/code-shiki
const getCodeLanguageOptionsShiki = (): [string, string][] => [];
const getCodeThemeOptionsShiki = (): [string, string][] => [];
const normalizeCodeLanguageShiki = (lang: string) => lang;

// Convert getCodeLanguages() (string[]) to [string, string][] format
const getCodeLanguageOptionsPrism = (): [string, string][] => {
  const languages = getCodeLanguages();
  return languages.map((lang: string) => [lang, lang] as [string, string]);
};
...
// Stub tags that don't exist in current Lexical version
...
import LineHeight, {parseLineHeightForToolbar} from './lineHeight';
import {
  clearFormatting,
  formatBulletList,
  formatCheckList,
...
export default function ToolbarPlugin({
  ...
  const $handleCodeNode = useCallback(
    (element: LexicalNode) => {
      if ($isCodeNode(element)) {
        ...
        // CodeNode doesn't have getTheme() in current version
        // const theme = element.getTheme();
        updateToolbarState('codeTheme', '');
        return;
      }
    },
  ...
  const $updateToolbar = useCallback(() => {
    ...
    if ($isRangeSelection(selection) || $isTableSelection(selection)) {
      ...
      const lineHeightValue = $getSelectionStyleValueForProperty(selection, 'line-height', '1.5');
      updateToolbarState(
        'lineHeight',
        parseLineHeightForToolbar(lineHeightValue) || '1.5',
      );
    ...
    (value: string) => {
      activeEditor.update(() => {
        if (selectedElementKey !== null) {
          const node = $getNodeByKey(selectedElementKey);
          if ($isCodeNode(node)) {
            // CodeNode doesn't have setTheme() in current version
            // node.setTheme(value);
            // Just update toolbar state for now
    ...
    <div className="toolbar">
      ...
      {toolbarState.blockType === 'code' && isCodeHighlighted ? (
        ...
        <>
          ...
          <Divider />
          <LineHeight
            selectionLineHeight={toolbarState.lineHeight}
            editor={activeEditor}
            disabled={!isEditable}
          />
          <Divider />
                ...
                <DropDownItem
                  onClick={() =>
                    insertGifOnClick({
                      ...
                      src: typeof catTypingGif === 'string' ? catTypingGif : (catTypingGif as any).src || '',
                    })
                  }
                  className="item">
