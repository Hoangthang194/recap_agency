...
// These are minimal implementations to prevent module resolution errors

const autoUpdate = () => () => {};
const flip = (options?: any) => ({ name: 'flip', options });
const offset = (value: number) => ({ name: 'offset', options: { mainAxis: value } });
const shift = (options?: any) => ({ name: 'shift', options });

const useDismiss = (context: any, options?: any) => ({});
const useFloating = (options: any) => ({
  refs: {
...
const useInteractions = (interactions: any[]) => ({
  getFloatingProps: () => ({}),
});
const useRole = (context: any, options?: any) => ({});

const FloatingFocusManager = ({ children, context, initialFocus }: any) => <>{children}</>;
const FloatingOverlay = ({ children, lockScroll }: any) => <>{children}</>;
const FloatingPortal = ({ children }: any) => <>{children}</>;
